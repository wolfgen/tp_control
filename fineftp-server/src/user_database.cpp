...
namespace fineftp
{
  ...
  bool UserDatabase::delUser(const std::string& username)
  {
	  std::lock_guard<decltype(database_mutex_)> database_lock(database_mutex_);
	  ...
	  if (isUsernameAnonymousUser(username))
	  {
		  if (!anonymous_user_)
		  {
			  std::cerr << "Error deleting user with username \"" << username << "\". The username denotes the anonymous user, which is not present." << std::endl;
			  return false;
		  }
		  else
		  {
              anonymous_user_ = nullptr;
			  return true;
		  }
	  }
	  else
	  {
		  ...
		  if (user_it != database_.end())
		  {
              database_.erase(user_it);
			  return true;
		  }
		  else
		  {
			  std::cerr << "Error deleting user with username \"" << username << "\". The user not exists." << std::endl;
			  return false;
		  }
	  }
  }
