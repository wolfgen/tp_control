                                 ...
                                 m_pMonitoringThread(NULL)
{
  ...
  BOOL bVistaOrLater = (GetVersionEx(&osvi) && osvi.dwMajorVersion >= 6);//IsWindowsVistaOrGreater();//(GetVersionEx(&osvi) && osvi.dwMajorVersion >= 6);
  if (bVistaOrLater)
  {
...
{
	CString _sDrive = sDrive;
	PathAddBackslash(_sDrive.GetBuffer(MAX_PATH));
	_sDrive.ReleaseBuffer();

  return GetVolumeInformation(_sDrive, NULL, 0, &dwSerialNumber, NULL, NULL, NULL, 0);
}
