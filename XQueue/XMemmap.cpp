...
#define XMM_MAPPING_FILENAME    _T("_MapFile")
                           ...
                           LPSECURITY_ATTRIBUTES lpSecurityAttributes)
{
    ...
    if (dwNumberOfBytesToMap) {
        m_nLength = dwNumberOfBytesToMap;
    } else {
        m_nLength = dwLength;
    }

    //Do the actual mapping
    CreateMappingName(lpszFilename, m_szMappingName, sizeof(m_szMappingName)/sizeof(TCHAR), bNamed);
                             ...
                             LPSECURITY_ATTRIBUTES lpSecurityAttributes)
{
    ...
    m_bReadOnly   = bReadOnly;
    m_bAppendNull = FALSE;
    CreateMappingName(lpszName, m_szMappingName, sizeof(m_szMappingName)/sizeof(TCHAR), TRUE);
#if _DEBUG_XMEMMAP
    TRACE(_T("m_szMappingName=%s\n"), m_szMappingName);
#endif
    return MapHandle(INVALID_HANDLE_VALUE, lpSecurityAttributes, 0);
}
...
BOOL CXMemMapFile::MapExistingMemory(LPCTSTR lpszName, DWORD dwBytes, BOOL bReadOnly)
{
    ...
    //Map the view
    m_lpData = MapViewOfFile(m_hMapping, dwDesiredAccess, 0, 0, m_nLength);
                             ...
                             DWORD dwStartOffset)
{
    ...
    m_hMapping = ::CreateFileMapping(hHandle, lpSecurityAttributes, flProtect, 0, (DWORD)nLength, m_szMappingName);
    if (m_hMapping == NULL) {
    ...
    m_lpData = MapViewOfFile(m_hMapping, dwDesiredAccess, 0, dwStartOffset, nLength);
                                      ...
                                      BOOL    bNamed)
{
    ...
    _ASSERTE(nSize > 2);
    if (!lpszMappingName || (nSize <= 2)) {
        return 0;
    }

    ZeroMemory(lpszMappingName, nSize*sizeof(TCHAR));

    if (bNamed) {
        _tcsncpy_s(lpszMappingName, nSize, lpszName, nSize-1);
        _tcsncat_s(lpszMappingName, nSize, XMM_MAPPING_FILENAME, _tcslen(XMM_MAPPING_FILENAME));
    }

    return (DWORD)(_tcslen(lpszMappingName));
}
...
DWORD CXMemMapFile::GetMappingName(LPTSTR lpszName, DWORD dwSize) const
{
    ...
    _tcsncpy_s(lpszName, dwSize, m_szMappingName, dwSize-1);

    return (DWORD)(_tcslen(lpszName));
}
