...
BOOL CTrayNotifyIcon::Hide()
{
    ...
    ATLASSERT(m_ShellVersion >= Version5); //Only supported on Shell v5 or later
    if (m_bHidden) { //Only makes sense to hide the icon if it is not already hidden
        return TRUE;
    }
...
BOOL CTrayNotifyIcon::Show()
{
    //Validate our parameters
    ATLASSERT(m_ShellVersion >= Version5); //Only supported on Shell v5 or later
    if (!m_bHidden) { //Only makes sense to show the icon if it has been previously hidden
        return TRUE;
    }
