...
public:
	BOOL PickFileRecord(ULONGLONG fileRef);
	BOOL ParseAttrs();
...
BOOL CFileRecord::PickFileRecord(ULONGLONG fileRef)
{
	// Clear previous data
	ClearAttrs();
	if (FileRecord)
...
CNTFSVolume::CNTFSVolume(_TCHAR volume)
{
	...
	if (!vol.PickFileRecord(MFT_IDX_VOLUME))
		return;
	...
	if (MFTRecord->PickFileRecord(MFT_IDX_MFT))
	{
