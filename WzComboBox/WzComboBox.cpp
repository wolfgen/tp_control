...
void CWzComboBox::DrawIconString(LPDRAWITEMSTRUCT lpDIS, BOOL bselected)
{
	...
	if (m_ImgList.GetImageCount() > 0)//has icon attach
	{
		POINT pt;
		CRect rcIcon;
	...
	else
	{
		if((lpDIS->itemState & ODS_COMBOBOXEDIT) && lpDIS->itemAction == ODA_DRAWENTIRE)
			...
			rcText.left = rcText.left + pData->cLevel * 20;//20 = 18+2
	}
...
int CWzComboBox::AddIcon(HICON hIcon)
{
	...
	return m_ImgList.Add(hIcon);//index = 0
}

WORD CWzComboBox::AddCTString(WORD wParentIdx,BYTE cType,LPCTSTR lpszString, LPCTSTR lpszName)
{
	int idx = -1;
...
//		int nLength = 20 + pData->cLevel * 8 + size.cx + nScrollWidth;
	...
	nWidth += pDC->GetTextExtent(_T("0")).cx;
